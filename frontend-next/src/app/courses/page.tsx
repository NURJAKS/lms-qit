...
export function CatalogPageContent({ embedded = false }: { embedded?: boolean }) {
  ...
  const [purchaseCredentials, setPurchaseCredentials] = useState<{
    temp_login: string;
    ...
    try {
      ...
      if (user && token && paymentId) {
        ...
        if (response && response.data) {
          ...
          setConfirmationToken(d.confirmation_token || null);
          setPurchaseCredentials({
            temp_login: d.temp_login,
            temp_password: d.temp_password,
            parent_temp_login: d.parent_temp_login,
            parent_temp_password: d.parent_temp_password,
