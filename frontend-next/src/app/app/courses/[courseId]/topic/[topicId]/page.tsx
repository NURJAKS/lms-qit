...
import { TopicLessonMaterialsSection } from "@/components/courses/TopicLessonMaterialsSection";
...
export default function TopicViewPage() {
  ...
  useEffect(() => {
    ...
    const st = searchParams.get("showTest");
    if (st === "true") {
      setShowTest(true);
    }
  }, [searchParams]);
  ...
  const { data: progressList = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get<Array<{ topic_id: number; is_completed: boolean; test_score: number | null; video_watched_seconds?: number }>>(`/progress/course/${cId}`);
      return data;
    },
    ...
    <div className="relative w-full max-w-4xl mx-auto px-3 sm:px-4 lg:px-0">
      ...
      {!showTest && (
        ...
        >
          ...
          {progress?.test_score !== null && progress?.test_score !== undefined && progress.test_score < 80 && (
            <button
              type="button"
            ...
            <>
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-yellow-700 dark:text-yellow-300 text-sm mb-2">
                        {t("videoDailyLimitMessage")
                          ?.replace("{used}", String(Math.floor(dailyVideoLimit.used_seconds / 60)))
                          ?.replace("{limit}", String(Math.floor(dailyVideoLimit.limit_seconds / 60)))}
                      </p>
                      <Link
                        href="/app/premium"
              ...
              {!isPremium && dailyVideoLimit && dailyVideoLimit.is_allowed && dailyVideoLimit.remaining_seconds < 1800 && (
                <div className="mb-4 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
                  <p className="text-blue-800 dark:text-blue-200 text-sm">
                    {t("videoDailyLimitWarning")?.replace("{remaining}", String(Math.floor(dailyVideoLimit.remaining_seconds / 60)))}
                  </p>
                </div>
              )}
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 mb-4">
                ...
                {!isPremium && dailyVideoLimit && (
                  <p className="text-sm text-gray-500">
                    {t("videoDailyLimitIndicator")?.replace("{used}", String(Math.floor(dailyVideoLimit.used_seconds / 60)))?.replace("{limit}", String(Math.floor(dailyVideoLimit.limit_seconds / 60)))}
                  </p>
                )}
              </div>
