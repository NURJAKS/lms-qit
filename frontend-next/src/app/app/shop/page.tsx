...
export default function ShopPage() {
  ...
  const { data: itemsData, isLoading: isItemsLoading } = useQuery({
    queryKey: ["shop-items", category, page],
    queryFn: async () => {
      if (category === "favorites") {
        const { data } = await api.get<ShopItem[]>("/shop/favorites");
        return { items: data, total_pages: 1 };
      }
      const baseUrl = category === "all" ? "/shop/items" : `/shop/items?category=${category}`;
      const sep = baseUrl.includes("?") ? "&" : "?";
      const { data } = await api.get<{ items: ShopItem[], pages: number }>(`${baseUrl}${sep}page=${page}&limit=${limit}`);
      return { items: data.items, total_pages: data.pages };
    },
  ...
  return (
    <div className="relative min-h-screen">
      <div className="relative z-10">
        ...
        <div className="grid grid-cols-1 gap-3 sm:gap-4 lg:gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        ...
        {totalPages > 1 && (
          <div className="mt-8 flex items-center justify-center gap-2">
            <button
              ...
              className="px-4 py-2 rounded-lg font-medium border border-gray-200 dark:border-gray-800 disabled:opacity-50 transition-colors"
