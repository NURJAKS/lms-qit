...
function LoginContent() {
  ...
  // Pre-fill form fields from URL params (e.g. ?email=...&password=...)
  useEffect(() => {
  ...
  const onSubmit = async (data: Form) => {
    ...
    try {
      ...
      // Если бэкенд сразу вернул пользователя, используем его (экономим 1 запрос)
      let userData = tokenData.user;
      if (!userData) {
        const { data: fetchedUser } = await api.get<User>("/users/me", {
  ...
  return (
    <div className="min-h-screen flex flex-col py-4 px-3 sm:px-6 sm:items-center sm:justify-center relative overflow-hidden">
      <div className="flex justify-end w-full max-w-[980px] mb-4 relative z-20">
        <AppHeader />
      </div>
        ...
        >
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <button
              ...
              className="w-full min-h-[44px] py-3 rounded-lg bg-indigo-600 text-white font-medium hover:bg-indigo-700 transition-colors touch-manipulation disabled:opacity-60 disabled:cursor-not-allowed"
            >
              {isSubmitting ? (
