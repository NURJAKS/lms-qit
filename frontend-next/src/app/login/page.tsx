...
function LoginContent() {
  ...
  const onGoogleSuccess = async (response: CredentialResponse) => {
    ...
    try {
      const { data: loginData } = await api.post<{
        user_exists: boolean;
        access_token?: string;
      }>("/auth/google-login", { id_token: response.credential });

      if (loginData.user_exists && loginData.access_token) {
  ...
  const handleRoleSelect = async (role: "student" | "teacher") => {
    ...
    try {
      ...
      setAuth(regData.user, regData.access_token);
      const redirect = searchParams.get("redirect");
      router.replace(redirect && redirect.startsWith("/") ? redirect : "/app");
    } catch (e: unknown) {
      setError(getErrorMessage(e));
    ...
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-[880px] bg-white dark:bg-gray-800 rounded-2xl shadow-2xl shadow-black/40 overflow-hidden flex flex-col lg:flex-row">
        ...
        <div className="lg:w-[55%] p-8 lg:p-10 flex flex-col justify-center">
          ...
          <Link
            href="/"
            className="block mt-3 text-sm text-indigo-600 hover:text-indigo-700 hover:underline"
          >
            {t("forgotPassword")}
          </Link>
...
function RoleModal({ isOpen, onSelect }: { isOpen: boolean; onSelect: (role: "student" | "teacher") => void }) {
  const { t } = useLanguage();
  if (!isOpen) return null;
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white dark:bg-gray-800 rounded-2xl p-8 max-w-md w-full shadow-2xl animate-in fade-in zoom-in duration-300">
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-6 text-center">{t("selectRole")}</h2>
        <div className="grid grid-cols-2 gap-4">
          <button
            onClick={() => onSelect("student")}
            className="flex flex-col items-center justify-center p-6 rounded-xl border-2 border-gray-100 dark:border-gray-700 hover:border-indigo-500 dark:hover:border-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition-all group"
          >
            <div className="w-12 h-12 rounded-full bg-indigo-100 dark:bg-indigo-900/40 flex items-center justify-center mb-3 group-hover:bg-indigo-200 dark:group-hover:bg-indigo-800/60 transition-colors">
              <GraduationCap className="w-6 h-6 text-indigo-600 dark:text-indigo-400" />
            </div>
            <span className="font-semibold text-gray-700 dark:text-gray-200">{t("studentRole")}</span>
          </button>
          <button
            onClick={() => onSelect("teacher")}
            className="flex flex-col items-center justify-center p-6 rounded-xl border-2 border-gray-100 dark:border-gray-700 hover:border-indigo-500 dark:hover:border-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition-all group"
          >
            <div className="w-12 h-12 rounded-full bg-violet-100 dark:bg-violet-900/40 flex items-center justify-center mb-3 group-hover:bg-violet-200 dark:group-hover:bg-violet-800/60 transition-colors">
              <BookOpen className="w-6 h-6 text-violet-600 dark:text-violet-400" />
            </div>
            <span className="font-semibold text-gray-700 dark:text-gray-200">{t("teacherRole")}</span>
          </button>
        </div>
      </div>
    </div>
