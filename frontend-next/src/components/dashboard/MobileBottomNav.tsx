...
export function MobileBottomNav() {
  ...
  return (
    <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-[1000] glass-nav border-t border-gray-200/50 dark:border-white/10 px-2 pb-safe">
        ...
        {navItems.map(({ href, icon: Icon, label, badge }) => {
          ...
          return (
            <Link
              key={href}
              href={href}
              className={`relative bottom-nav-item active-tap flex flex-col items-center justify-center min-w-0 flex-1 px-0.5 transition-all duration-300 ${
                isActive 
                  ? "text-[var(--qit-accent)] dark:text-[var(--qit-accent)] scale-105" 
                  : "text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300"
              }`}
            >
              <div className={`relative p-1 rounded-xl transition-all duration-300 ${isActive ? "bg-[var(--qit-accent)]/10" : ""}`}>
                <Icon className={`w-5 h-5 sm:w-6 sm:h-6 ${isActive ? "drop-shadow-[0_0_8px_rgba(255,64,129,0.5)]" : ""}`} />
                {badge !== undefined && (
              ...
              <span
                className={`mt-0.5 max-w-full truncate text-center text-[9px] font-bold transition-all duration-300 ${isActive ? "opacity-100" : "opacity-80"}`}
                title={label}
              >
                {label}
              </span>
