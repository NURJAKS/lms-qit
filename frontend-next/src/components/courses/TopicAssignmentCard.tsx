...
import { CheckCircle2, FileText, Globe, Loader2, Plus, X, Sparkles, ChevronDown, ChevronUp } from "lucide-react";
...
export function TopicAssignmentCard({
  ...
  // Default expansion: unsubmitted assignments are expanded, submitted are collapsed
  const [isExpanded, setIsExpanded] = useState(initialExpanded !== undefined ? initialExpanded : !assignment.submitted);
  ...
  const unsubmitMutation = useMutation({
    ...
    onSuccess: () => {
      setActionError(null);
      refreshData();
      setIsExpanded(true); // Expand when unsubmitting
    },
    onError: (err) => {
  ...
  return (
    <article className={cn(
      "rounded-2xl border transition-all p-4 sm:p-5",
      isGraded 
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-3 min-w-0">
          <div className={cn(
            ...
            isGraded ? "bg-green-100 dark:bg-green-900/40" : "bg-blue-50 dark:bg-blue-900/40"
          )}>
            <ClipboardListIcon className={cn("w-4.5 h-4.5", isGraded ? "text-green-600 dark:text-green-400" : "text-blue-600 dark:text-blue-400")} />
          </div>
          <div className="min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-2 mt-0.5">
              <span className={cn(
                ...
                isGraded ? "text-green-600 dark:text-green-400" : assignment.submitted ? "text-blue-600 dark:text-blue-400" : "text-gray-500 dark:text-gray-400"
              )}>
        ...
        <button
          ...
          className="shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/50 dark:bg-black/20 text-sm font-bold text-gray-600 dark:text-gray-400 hover:bg-white dark:hover:bg-black/40 transition-all border border-gray-100 dark:border-gray-800"
        >
          {isExpanded ? (
            <>
