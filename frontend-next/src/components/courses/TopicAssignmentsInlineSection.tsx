              ...
              {question.type === "single_choice" && question.options && question.options.length > 0 ? (
                <div className="space-y-2">
                  {question.options.map((opt, idx) => (
                    <label key={idx} className="flex items-start gap-3 p-3 sm:p-3.5 border dark:border-gray-700 rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 has-[:checked]:bg-blue-500/10 has-[:checked]:border-blue-500 min-h-[2.75rem]">
                      <input
                        type="radio"
                        name={`q-${question.id}`}
                        value={opt}
                        ...
                        className="w-4 h-4 shrink-0 mt-0.5 accent-blue-600"
            ...
            <div className="mt-4 p-3 bg-gray-50 dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700">
              ...
              {question.type === "single_choice" && question.options && question.options.length > 0 ? (
                <div className="space-y-2 mt-2">
                  {question.options.map((opt, idx) => {
                    const isSelected = question.answer_text === opt;
                    const isCorrect = isGraded && question.correct_option === opt;
                    const isWrong = isGraded && isSelected && question.correct_option && question.correct_option !== opt;
                    
                    return (
                      <div key={idx} className={cn(
                        "flex items-start gap-3 p-3 rounded-lg border",
                        isSelected && !isGraded ? "border-blue-500 bg-blue-500/10" : 
                        isCorrect ? "border-green-500 bg-green-500/10" :
                        isWrong ? "border-red-500 bg-red-500/10" :
                        "border-gray-200 dark:border-gray-700"
                      )}>
                        <div className="w-4 h-4 shrink-0 mt-0.5 rounded-full border-2 flex items-center justify-center bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-600">
