...
export function TestComponent({ testId, onComplete, onCancel, onRetake, passedContinueLabelKey }: TestComponentProps) {
  ...
  if (submitMutation.isSuccess && submitMutation.data) {
    ...
    if (tier === "passed") {
      return (
        <div className="fixed inset-0 z-[2000] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md overflow-y-auto">
          <div className="w-full max-w-md mx-auto rounded-3xl overflow-hidden shadow-2xl border border-gray-100 dark:border-gray-800 bg-white dark:bg-slate-900 p-6 sm:p-8 text-center animate-in fade-in zoom-in duration-300">
            {/* Elegant Circular Checked Icon with Soft Glow */}
            <div className="mx-auto w-16 h-16 rounded-full bg-green-50 dark:bg-green-950/30 flex items-center justify-center text-green-600 dark:text-green-400 mb-5 shadow-inner shadow-green-500/10">
              <span className="text-3xl">✅</span>
            </div>
            
            <h2 className="text-xl sm:text-2xl font-black text-gray-900 dark:text-white tracking-tight mb-2">
              {t("testResultPassedTitle")}
            </h2>
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-6 px-2 leading-relaxed">
              {t("testResultPassedDesc")}
            </p>
            
            {/* Score Display Card */}
            <div className="bg-gray-50 dark:bg-slate-950/50 rounded-2xl p-5 border border-gray-100 dark:border-gray-800/80 mb-8 max-w-[280px] mx-auto">
              <span className="text-xs font-bold text-gray-400 uppercase tracking-widest block mb-1">
                {t("testScore")}
              </span>
              <span className="text-5xl font-black text-green-600 dark:text-green-400 tracking-tight">
                {d.score.toFixed(0)}%
              </span>
              <span className="text-xs font-semibold text-gray-400 dark:text-gray-500 block mt-2">
                {d.correct_count} / {d.total_count} {t("testQuestion").toLowerCase()}
              </span>
            </div>
            ...
            <button 
              type="button" 
              onClick={onComplete} 
              className="w-full py-3.5 px-6 rounded-xl text-white font-extrabold text-sm transition-all shadow-lg active:scale-[0.98] duration-200"
              style={{ 
      ...
      return (
        <div className="fixed inset-0 z-[2000] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md overflow-y-auto">
          <div className="w-full max-w-md mx-auto rounded-3xl overflow-hidden shadow-2xl border border-gray-100 dark:border-gray-800 bg-white dark:bg-slate-900 p-6 sm:p-8 text-center animate-in fade-in zoom-in duration-300">
            ...
            <div className="mx-auto w-16 h-16 rounded-full bg-amber-50 dark:bg-amber-950/30 flex items-center justify-center text-amber-500 dark:text-amber-400 mb-5 shadow-inner shadow-amber-500/10">
              <span className="text-3xl">⚠️</span>
            </div>
            
            <h2 className="text-xl sm:text-2xl font-black text-gray-900 dark:text-white tracking-tight mb-2">
              {t("testResultNeedsReviewTitle")}
            </h2>
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-6 px-2 leading-relaxed">
              {t("testResultNeedsReviewDesc")}
            </p>
            
            {/* Score Display Card with Progress Bar */}
            <div className="bg-gray-50 dark:bg-slate-950/50 rounded-2xl p-5 border border-gray-100 dark:border-gray-800/80 mb-6">
              <div className="flex items-center justify-between mb-3">
                <span className="text-xs font-bold text-gray-400 uppercase tracking-widest">{t("testScore")}</span>
                <span className="text-2xl font-black text-amber-500">{d.score.toFixed(0)}%</span>
              </div>
              <TestScoreThresholdBar score={d.score} barClassName="bg-amber-500" marker50ClassName="text-amber-500" />
            </div>
            
            <p className="text-xs text-gray-400 dark:text-gray-500 mb-8 leading-relaxed max-w-[320px] mx-auto">
              {t("testResultNeedsReviewSupplementaryHint")}
            </p>
            
            {/* Beautiful, non-conflicting buttons */}
              ...
              {showSuppLink ? (
                <>
                  ...
                  <button 
                    ...
                    className="w-full py-3 px-6 rounded-xl bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700/80 text-gray-700 dark:text-gray-300 font-extrabold text-sm transition-all active:scale-[0.98] duration-200"
                  >
                    {t("testRetry")}
                  </button>
                </>
    ...
    return (
      <div className="fixed inset-0 z-[2000] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md overflow-y-auto">
        <div className="w-full max-w-md mx-auto rounded-3xl overflow-hidden shadow-2xl border border-gray-100 dark:border-gray-800 bg-white dark:bg-slate-900 p-6 sm:p-8 text-center animate-in fade-in zoom-in duration-300">
          {/* Failed Cross Icon with Soft Glow */}
          <div className="mx-auto w-16 h-16 rounded-full bg-red-50 dark:bg-red-950/30 flex items-center justify-center text-red-600 dark:text-red-400 mb-5 shadow-inner shadow-red-500/10">
            <span className="text-3xl">❌</span>
          </div>
          
          <h2 className="text-xl sm:text-2xl font-black text-gray-900 dark:text-white tracking-tight mb-2">
            {t("testResultFailedTitle")}
          </h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-6 px-2 leading-relaxed">
            {t("testResultFailedDesc")}
          </p>
          ...
          <div className="bg-gray-50 dark:bg-slate-950/50 rounded-2xl p-5 border border-gray-100 dark:border-gray-800/80 mb-6">
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs font-bold text-gray-400 uppercase tracking-widest">{t("testScore")}</span>
              <span className="text-2xl font-black text-red-600 dark:text-red-500">{d.score.toFixed(0)}%</span>
            </div>
            <TestScoreThresholdBar score={d.score} barClassName="bg-red-500" marker50ClassName="text-red-500" />
          </div>
          
          <p className="text-xs text-gray-400 dark:text-gray-500 mb-8 leading-relaxed max-w-[320px] mx-auto">
            {t("testResultFailedSupplementaryHint")}
          </p>
  ...
  return (
    <div
      ref={containerRef}
      className="fixed inset-0 z-[2000] flex flex-col items-center bg-white dark:bg-gray-900 overflow-y-auto p-4 pb-[max(1rem,env(safe-area-inset-bottom))] pt-[max(1rem,env(safe-area-inset-top))] transition-all duration-300"
    >
      {!isFullscreen && !submitMutation.isSuccess && (
        <div className="absolute inset-0 z-[2010] flex flex-col items-center justify-center bg-white dark:bg-gray-900 px-4 py-8 sm:p-6 text-center text-gray-800 dark:text-white">
      ...
      {showCheatingWarning && !submitMutation.isSuccess && (
        <div className="absolute inset-0 z-[2020] flex flex-col bg-black/90 backdrop-blur-md px-4 pt-[max(1.25rem,env(safe-area-inset-top))] pb-[max(1.25rem,env(safe-area-inset-bottom))] text-center text-white sm:items-center sm:justify-center sm:p-6">
      ...
      {showExitConfirm && !submitMutation.isSuccess && (
        <div className="absolute inset-0 z-[2030] flex flex-col items-center justify-center bg-black/80 backdrop-blur-sm p-4 sm:p-6 text-center text-white">
