...
from app.services.activity_log import log_activity, log_activity_no_commit
...
def login(request: Request, data: UserLogin, db: Session = Depends(get_db)):
    ...
    log_activity_no_commit(db, user.id, "login", "user", user.id, {"email": user.email})
    db.commit()
    token = create_access_token(data={"sub": str(user.id), "email": user.email, "role": user.role})
    
    # Прямо здесь формируем UserResponse для экономии запроса с фронта
    from app.api.routes.users import get_me
    user_data = get_me(db, user)
