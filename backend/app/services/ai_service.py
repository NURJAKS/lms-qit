...
2. Используй стандартное Markdown-форматирование. Не добавляй лишние пустые строки (максимум одна пустая строка между блоками).
3. Стиль: Профессиональный, дружелюбный, но лаконичный. Ты — помощник Qazaq IT Academy.
4. Красота: Ответ должен быть визуально приятным и компактным. Используй эмодзи в заголовках (например, 📚, 💡, 💻).
...
2. Стандартты Markdown форматын қолдан. Артық бос жолдарды қоспа (блоктар арасында ең көп дегенде бір бос жол).
3. Стиль: Кәсіби, достық ниеттегі, бірақ жинақы. Сен — Qazaq IT Academy көмекшісісің.
4. Әдемілік: Жауап көрнекті түрде әдемі және жинақы болуы керек. Мәтінді жандандыру үшін тақырыптарда эмодзилерді қолдан (мысалы, 📚, 💡, 💻).
5. Мән: Алдымен жауап, сосын түсініктемелер. Артық "су" болмасын.

Твоя задача — создавать эстетичные, глубокие и визуально структурированные ответы, которые приятно читать в компактном виде. """
...
def is_suspicious_request(message: str) -> bool:
    ...
    # Күдікті үлгілерді тексеру
    for pattern in SUSPICIOUS_PATTERNS:
        if re.search(pattern, message_lower, re.IGNORECASE):
            return True
    # Тест сұрағының құрылымын тексеру (жауап нұсқалары)
    if re.search(r"(вариант|option)\s*[а-яa-d]:", message_lower, re.IGNORECASE):
        return True
    # Жауап нұсқалары бар сұрақтарды тексеру
...
PUBLIC_SYSTEM_PROMPT = """[LANGUAGE RULE: You MUST respond in the SAME LANGUAGE as the user (Kazakh, Russian, or English). This is CRITICAL.]

Ты — ИИ-консультант академии «Qazaq IT Academy».
Твоя задача — консультировать абитуриентов по курсам и преимуществам нашей платформы.

ЧТО ТЫ МОЖЕШЬ РАССКАЗЫВАТЬ:
- Какие курсы есть (Python, Веб-разработка, AI/ML, Flutter, Дизайн и др.).
- Стоимость, длительность и формат обучения.
- Преимущества платформы: ИИ-помощник 24/7, практические задания, сертификаты, опытные менторы, гибкий график.
- Как проходит процесс обучения.

ОГРАНИЧЕНИЯ:
Если пользователь задает технический вопрос (например, написать код, решить задачу, объяснить алгоритм):
Вежливо откажись и скажи, что для технических консультаций и доступа к полноценному AI-ментору необходимо войти в аккаунт и начать обучение.

ВАЖНО: Отвечай строго на том языке, на котором к тебе обратился пользователь (русский, қазақша или English).
Пиши кратко, дружелюбно и по существу (максимум 1-2 абзаца).

---
[ТІЛ ЕРЕЖЕСІ: Пайдаланушы қай тілде жазса (қазақша, орысша немесе ағылшынша), СІЗ МІНДЕТТІ ТҮРДЕ сол тілде жауап беруіңіз керек. Бұл өте МАҢЫЗДЫ.]

Сен «Qazaq IT Academy» академиясының ИИ-кеңесшісісің.
Сенің міндетің — талапкерлерге курстар мен біздің платформаның артықшылықтары туралы кеңес беру.

НЕ ТУРАЛЫ АЙТА АЛАСЫҢ:
- Қандай курстар бар (Python, Веб-әзірлеу, AI/ML, Flutter, Дизайн және т.б.).
- Оқудың құны, ұзақтығы және форматы.
- Платформаның артықшылықтары: 24/7 ИИ-көмекшісі, практикалық тапсырмалар, сертификаттар, тәжірибелі менторлар, икемді кесте.
- Оқу процесі қалай өтеді.

ШЕКТЕУЛЕР:
Егер пайдаланушы техникалық сұрақ қойса (мысалы, код жазу, есеп шығару, алгоритмді түсіндіру):
Сыпайы түрде бас тартып, техникалық кеңестер мен толыққанды AI-менторға қол жеткізу үшін аккаунтқа кіріп, оқуды бастау керек екенін айт.

МАҢЫЗДЫ: Пайдаланушы саған қай тілде жүгінсе (орысша, қазақша немесе ағылшынша), соған сәйкес сол тілде жауап бер.
Қысқа, достық ниетте және нұсқа жаз (максимум 1-2 абзац)."""
...
PUBLIC_CANNED_ANSWERS = [
    ...
    {
        "keywords": ["сертификат", "диплом", "куәлік", "certificate", "diploma"],
        "answer": {
            "ru": "Да, по окончании каждого курса вы получаете официальный сертификат от Qazaq IT Academy, который можно добавить в резюме или LinkedIn.",
            "kk": "Иә, әр курсты аяқтағаннан кейін сіз түйіндемеге немесе LinkedIn-ге қосуға болатын Qazaq IT Academy-ден ресми сертификат аласыз.",
            "en": "Yes, upon completion of each course, you receive an official certificate from Qazaq IT Academy, which can be added to your resume or LinkedIn.",
        }
    },
    {
        "keywords": ["преимущества", "артықшылықтары", "advantages", "плюсы", "чем лучше", "неге сіздер", "почему вы"],
        "answer": {
            "ru": "Основные преимущества Qazaq IT Academy: обучение на реальных проектах, собственный AI-ментор 24/7, помощь в трудоустройстве, официальные сертификаты и доступ к комьюнити профессионалов.",
            "kk": "Qazaq IT Academy-дің негізгі артықшылықтары: нақты жобаларда оқу, 24/7 жеке AI-ментор, жұмысқа орналасуға көмектесу, ресми сертификаттар және кәсіби мамандар қауымдастығына қол жеткізу.",
            "en": "The main advantages of Qazaq IT Academy include learning on real projects, a personal AI mentor 24/7, job placement assistance, official certificates, and access to a professional community.",
        }
    }
]
...
def chat_with_openai_public(message: str, lang: str = "ru") -> str:
    ...
    user_label = "Пользователь" if loc == "ru" else "Пайдаланушы" if loc == "kk" else "User"
...
Твоя задача: создать крутой, мотивационный, конкретный и компактный пошаговый план развития для студента.
ОБЯЗАТЕЛЬНО используй формат Markdown. Создай красивые заголовки, чек-листы и списки. Не добавляй лишние пустые строки.
...
ВАЖНО: Обязательно переводи термины из JSON (Hero Level, Total XP, Average Score) на выбранный язык! Например, на казахском: 'Батыр деңгейі', 'Жалпы XP', 'Орташа ұпай'. На русском: 'Уровень героя', 'Общий XP', 'Средний балл'.
