...
def synopsis_graded(db: Session, user_id: int, topic_id: int) -> bool:
    ...
    if not topic:
        return False
    gids = student_group_ids_for_course(db, user_id, topic.course_id)
    if not gids:
        return False

    # Find main synopsis assignment assigned to this student's groups
...
def topic_assignments_for_student(
    ...
    return (
        ...
        .filter(
            ...
            TeacherAssignment.group_id.in_(gids),
            TeacherAssignment.is_synopsis == False,
            TeacherAssignment.is_supplementary == False,
        )
        .all()
    )
...
def topic_questions_for_student(
    db: Session, user_id: int, topic_id: int, course_id: int
    ...
    return (
        ...
        .filter(
            TeacherQuestion.topic_id == topic_id,
            TeacherQuestion.course_id == course_id,
            TeacherQuestion.group_id.in_(gids),
        )
        .all()
    )


def all_questions_graded(
    db: Session, user_id: int, questions: list[TeacherQuestion]
) -> bool:
    if not questions:
        return True
    for q in questions:
        ans = (
            ...
            .filter(
                TeacherQuestionAnswer.student_id == user_id,
                TeacherQuestionAnswer.question_id == q.id,
            )
            .first()
        )
        if ans is None or ans.grade is None:
            return False
    return True
...
def can_take_topic_test(db: Session, user_id: int, topic_id: int, course_id: int) -> tuple[bool, str]:
    ...
    questions = topic_questions_for_student(db, user_id, topic_id, course_id)
    if questions and not all_questions_graded(db, user_id, questions):
        return False, "wait_grade"
    return True, "ok"
...
def topic_flow_status(db: Session, user_id: int, topic: CourseTopic) -> dict:
    ...
    questions = topic_questions_for_student(db, user_id, topic.id, course_id)
    has_hw = len(assigns) > 0 or len(questions) > 0
    graded_ok = True
    ...
    return {
        ...
        "topic_question_ids": [q.id for q in questions],
    }
