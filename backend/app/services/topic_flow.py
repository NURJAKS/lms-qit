...
def can_take_topic_test(db: Session, user_id: int, topic_id: int, course_id: int) -> tuple[bool, str]:
    ...
    if not video_requirement_met(db, user_id, topic):
        return False, "video"
    if not synopsis_graded(db, user_id, topic_id):
...
def topic_test_gate_message(reason: str) -> str:
    return {
        ...
        "video": "Сначала досмотрите видео по теме до конца.",
        "synopsis": "Загрузите конспект по теме и дождитесь его проверки учителем/куратором.",
