...
def send_course_purchase_email(
  ...
  """
  Welcome-письмо после мгновенной покупки: зачисление + вход.
  Содержит логин/пароль и подсказку об изменении пароля.
  """
  ...
  login_label = get_email_translation("login", lang)
  password_label = get_email_translation("password", lang)
  password_hint = get_email_translation("password_change_hint", lang)
  ...
  <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #0F172A; border-radius: 16px; overflow: hidden;">
    ...
    <div style="padding: 32px; color: #E2E8F0;">
      ...
      <p style="color: #94A3B8; font-size: 13px; line-height: 1.5; margin: 0 0 24px 0; padding: 12px 16px; background: #1E293B; border-radius: 8px; border-left: 3px solid #60A5FA;">
        💡 {password_hint}
      </p>
      {parent_credentials_html}
